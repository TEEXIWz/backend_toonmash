...
router.get('/date', (req, res)=>{
    if (req.query.id) {
        ...
        conn.query('SELECT SUM(score) as totalScore,DATE(voted_at) as date FROM vote WHERE pid = ? and voted_at BETWEEN DATE_SUB(NOW(),INTERVAL 7 DAY) and now() GROUP BY DATE(voted_at) ORDER BY DATE(voted_at) desc',req.query.id, (err,result)=>{
            if (err) {
                res.status(500).json(err)
            }
            if (result.length) {
